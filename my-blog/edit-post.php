...
if (isset($_POST['submit'])) {
  ...
  if (empty($title)) {
    ...
    $error = ' * article field is required';
      ...
      $newPost = $database->update('posts',$data, $postId);
...
if($postId){
  $database = new Database();
  $database->connect();
  $post = $database->select("SELECT * FROM posts WHERE id='$postId'")[0];
}
      ...
      <main class="py-4">
        <form method="post" action="" novalidate="novalidate">
          ...
          <div>
            ...
            <textarea name="article" id="article">  <?= $post['article']; ?>   </textarea>
          </div>
